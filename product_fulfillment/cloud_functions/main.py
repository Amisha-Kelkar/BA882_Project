...
#     project = os.getenv("GOOGLE_CLOUD_PROJECT", "ba882-team4")
...
#     """Weekly incremental loader for Target API → MotherDuck (with one-row-per-service flattening)."""

#     # --- Load secrets ---
#     md_token = get_secret("mother_duck_amisha")
...
#     conn.execute("CREATE TABLE IF NOT EXISTS raw_target_api AS SELECT * FROM (SELECT 1 AS dummy) WHERE 1=0")

#     # --- Check already loaded record_dates ---
#     try:
#         existing_dates = set(r[0] for r in conn.execute("SELECT DISTINCT load_date FROM raw_target_api").fetchall())
#     except Exception:
#         existing_dates = set()

#     print(f"Already loaded record_dates: {existing_dates}")
...
#     if not days_to_load:
#         print("No new record_date to load. Exiting cleanly.")
#         conn.close()
...
#     # Flatten 'services' list (→ one row per service) 
#     expanded_json = []
#     for record in all_json:
#         # Safely extract the deeply nested services list
#         services_list = (
#             record.get("data", {})
#             .get("product", {})
#             .get("fulfillment", {})
...
#     dedup_cols = ["store_id", "tcin", "load_date", "services_shipping_method_id"]
...
#     # conn.execute("CREATE OR REPLACE TABLE raw_target_api AS SELECT * FROM df_view")
#     conn.execute("""
#     CREATE TABLE IF NOT EXISTS raw_target_api AS 
#     SELECT * FROM df_view WHERE 1=0
# """)
#     conn.execute("""
#     INSERT INTO raw_target_api
#     SELECT DISTINCT * FROM df_view
# """)
...
#                     data_product_fulfillment_product_id::STRING AS product_id,
...
#     conn.close()
#     return (f"Loaded {len(df)} rows for {len(days_to_load)} new record_dates.", 200)
